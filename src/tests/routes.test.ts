...
describe('route tests', () => {
  ...
  describe('users tests', () => {
    ...
    test('if can list all users', async () => {
      ...
      const listUsers = await request(app.server)
        ...
        .send()
        .expect(200)
      expect(listUsers.body.userTables).toEqual([
  ...
  describe('like test', () => {
    test('if can create a like', async () => {
      ...
      const token = login.body.authToken
      const createLike = await request(app.server)
        .post(`/likes/${userId}`)
        .set('Authorization', `Bearer ${token}`)
        .send({
          productLiked: '32',
  ...
  test('if can delete likes', async () => {
    ...
    const deleteLike = await request(app.server)
      .delete(`/likes/${userId}`)
      .set('Authorization', `Bearer ${token}`)
      .send({ productLiked: '32' })
      .expect(202)
  ...
  test('if can list likes', async () => {
    ...
    const listLikes = await request(app.server)
      .get(`/likes/${userId}`)
      .set('Authorization', `Bearer ${token}`)
      .send()
      .expect(200)
